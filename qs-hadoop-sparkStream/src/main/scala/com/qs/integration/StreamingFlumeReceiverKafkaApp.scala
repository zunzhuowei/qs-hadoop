...
/**
  * spark streaming 集成kafka Receiver-based Approach 模式 以及集成flume 收集logger集成
  *
  */
object StreamingFlumeReceiverKafkaApp {

  def main(args: Array[String]): Unit = {

    if (args.length != 4) {
      println("StreamingFlumeReceiverKafkaApp usage " +
    ...
    val sparkConf = new SparkConf()
      .setAppName("StreamingFlumeReceiverKafkaApp")
      .setMaster("local[2]")
